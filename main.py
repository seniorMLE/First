...
def trainAgent(agent, loss_criteria):
    global global_step

    trainer=Trainer(agent, PlaceCells_units, numberSteps) # numberSteps=800,
    ...
    while (global_step<SGDSteps):
        ...
        inputData, pos, angle=dataGenerator.generateData(batch_size=num_trajectories)  # num_trajectories = 500
        

        labelData=np.zeros((num_trajectories, numberSteps, PlaceCells_units + HeadCells_units))  # numberSeps=800
        ...
        for startB in range(0, num_trajectories, batch_size):  #batch_size = 10
            ...
            bool_met_criteria, mn_Loss = trainer.training(batchX, batchY, global_step, loss_criteria)  # training model
            
            print("Global Step:")
            print(global_step)
            
            if bool_met_criteria == True:
                print("Because loss value met the criteria: ", mn_Loss)
                break                
...
if __name__ == '__main__':
    ...
    try:         
        ...
        agent=Network(lr=learning_rate, hu=LSTMUnits, lu=linearLayerUnits, place_units=PlaceCells_units, head_units=HeadCells_units, clipping=clipping, weightDecay=weightDecay, batch_size=batch_size, num_features=num_features, n_steps=numberSteps)
        ...
        if my_dir.is_dir():
           ...
           print("Model updated at global step:", agent.epoch, "loaded")
        #    global_step=agent.epoch
        
        loss_criteria = 0.608   
        if(args.mode == "train"):    
            trainAgent(agent, loss_criteria)
